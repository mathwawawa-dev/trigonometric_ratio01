...
def draw(vertices_dict, right_v, side_labels, filename,
    ...
    for (v1, v2, label, ap, peak) in arcs:
        ...
        pixels_per_du = np.linalg.norm(p1 - p0)
        pts_per_du = pixels_per_du * (72.0 / fig.dpi)
        if pts_per_du > 0:
        ...
        for seg, is_post_gap in segments:
            ...
            if _R < _dash_on_du:
                target_dist = _total - _R
                keep_idx = _dist_du <= target_dist
                pts_kept = pts_draw[keep_idx]
                if len(pts_kept) > 0 and len(pts_draw) > len(pts_kept):
                    idx_before = len(pts_kept) - 1
                    idx_after = len(pts_kept)
                    pA = pts_draw[idx_before]
                    pB = pts_draw[idx_after]
                    dA = _dist_du[idx_before]
                    dB = _dist_du[idx_after]
                    if dB > dA:
                        t = (target_dist - dA) / (dB - dA)
                        p_interp = pA + t * (pB - pA)
                        pts_draw = np.vstack((pts_kept, p_interp))
                    else:
                        pts_draw = pts_kept
                else:
                        pts_draw = pts_kept
