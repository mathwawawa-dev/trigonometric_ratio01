...
_CLR_TABLE = {
    ...
    3: dict(L=16.5, R=20.5, T=19.5, B=12.0),
    4: dict(L=21.5, R=24.5, T=19.5, B=12.0),
    5: dict(L=26.5, R=25.5, T=19.5, B=12.0),
    6: dict(L=30.5, R=30.5, T=19.5, B=12.0),
}
...
def draw(vertices_dict, right_v, side_labels, filename,
    ...
    if highlight_angle and highlight_angle in vertices_dict and highlight_angle != right_v:
        ...
        # 기본 비율 7% | 30도 미만 뾰족한 각은 최대 13%까지 선형 증가
        if angle_diff < 30:
            ratio = 0.13 + (30 - angle_diff) * 0.003
        elif angle_diff < 45:
            ratio = 0.07 + (45 - angle_diff) * 0.004
        else:
            ratio = 0.07
